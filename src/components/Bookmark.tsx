...
const Bookmark: React.FC<BookmarkProps> = ({ articleId }) => {
    ...
    useEffect(() => {
        const fetchBookmarkedArticles = async () => {
            try {
                const response = await axiosInstance.post(`${apiUrl}/api/bookmark/articles/${articleId}`);
    ...
    const handleBookmarkClick = async () => {
        try {
            if (isBookmarked) {
                ...
                await axiosInstance.delete(`${apiUrl}/api/bookmark/delete-bookmark/${articleId}`);
                setIsBookmarked(false);
            } else {
                await axiosInstance.post(`${apiUrl}/api/bookmark/add-bookmark/${articleId}`);
                setIsBookmarked(true);
            }
        } catch (err) {
