    ...
    const Register: React.FC = () => {
        ...
        const handleSendEmail = () => {
            ...
            // 중복 확인 및 이메일 발송 API 요청
            axios.post('http://localhost:8080/member/check-user', { username: userId, email: email, nickname: nickname })
                .then(response => {
                    if (response.data.exists) {
                        // 중복된 필드가 있을 경우 처리
                        const { userIdExists, emailExists, nicknameExists } = response.data;
                        setErrors(prev => ({
                            ...prev,
                            userId: userIdExists,
                            email: emailExists,
        ...
        const handleVerifyCode = () => {
            ...
            // 인증 코드 확인 API 요청
            axios.post('/api/verify-code', { code: userDetails.code })
                .then(response => {
        ...
        const handleRegister = () => {
            const { userId, password, confirmPassword, nickname, email } = userDetails; // email 추가
            ...
            if (isEmpty(userId) || isEmpty(password) || isEmpty(confirmPassword) || isEmpty(nickname) ||
                ...
                setErrors(prev => ({
                    ...
                    nickname: isEmpty(nickname) // nickname 오류 처리 추가
                }));
